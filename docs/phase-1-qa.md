...
Manual inspection path for the first lead and admin happy path.

## Public Lead Flow

- Open `/no` and confirm the page presents Senja Malere in Norwegian.
- Submit the lead form with required fields only: name, phone, area/city, service type, project description, and consent.
...
- Confirm honeypot submissions do not create normal leads.
...
- Confirm leads appear newest first and can be filtered by status.
- Open a lead detail page and confirm submitted contact/project fields are visible.
- Change a lead status and confirm the status update persists and creates status history.

## Analytics

- Visit public pages and confirm page-view events are recorded without making public pages dynamic.
- Submit a lead and confirm a `lead_submitted` event is recorded.
